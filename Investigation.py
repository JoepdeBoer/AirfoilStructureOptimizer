...
#subplots Created with Codeium AI Assistant. (Version 1.0) [Software]. (2021). Retrieved from https://www.codeium.com
    ...
    points = 50
    thickness = np.linspace(0.0005, 0.01, points*10)
    spar1 = np.linspace(0.4, 0.6, points)
    spar2 = np.linspace(0.45, 0.61, points)
    bigM = np.zeros((points, points))
    bigO = np.zeros((points, points))
    l1 = []

    for i, t in enumerate(thickness):
        airfoil = Airfoil("NACA012.txt", t, [0.98, 0.99], 2.5, [0, 75 * 10 ** 3, 300 * 10 ** 3, 1])
    ...
    for j, x1 in enumerate(spar1):
        for k, x2 in enumerate(spar2):
                if x2 > x1 + 0.01:
                    airfoil = Airfoil("NACA012.txt", t, [x1, x2], 2.5, [0, 75 * 10 ** 3, 300 * 10 ** 3, 1])
                    shear_stress = airfoil.find_max_shear_flow()/t
                    bigM[k, j] = (abs(shear_stress) - 207 * 10 ** 6)/(207 * 10 ** 6)
                    bigO[k, j] = airfoil.structural_area()
                else:
                    bigM[k, j] = np.NaN
                    bigO[k, j] = np.NaN
    ...
    ax.plot_surface(Spar1, Spar2, bigM, cmap='viridis')
    ax.set_xlabel(r'$x_{1}$ $[-]$')
    ax.set_ylabel('$x_{2}$ $[-]$')
    ax.set_zlabel(r'$g_{5}$ $[-]$')


    plt.show()


    #Create a 3D surface plot of spar1, spar2, and bigO
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.plot_surface(Spar1, Spar2, bigO, cmap='viridis')
    ax.set_xlabel(r'$x_{1}$ $[-]$')
    ax.set_ylabel('$x_{2}$ $[-]$')
